...
export function transformComponentSchemas(context, models) {
    ...
    return;
    function transformComponentSchema(types, name, context, schema) {
    ...
    function populateModel(types, rawName, context, schema) {
        const { name, scope } = getScopeAndName(rawName);
        ...
        const isModelReferencedAsMultipartRequestBody = context.isSchemaReferenceRegisteredForMultipartForm(refName);
     ...
     */
    function extractUnionMetadata(schema) {
        ...
        if (unionMembers) {
            const meaningfulMembers = unionMembers.filter((member) => {
                if ("$ref" in member)
                    return true; // Redundant, but help the type system understand whether it's a ref or not, and give us access to the type property
