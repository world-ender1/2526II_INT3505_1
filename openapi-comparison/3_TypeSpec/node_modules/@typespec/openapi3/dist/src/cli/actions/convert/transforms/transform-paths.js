...
import { getDirectivesForSchema, getExtensions, getParameterDecorators, } from "../utils/decorators.js";
...
export function transformPaths(paths, context) {
    ...
    for (const route of Object.keys(paths)) {
        const routeParameters = paths[route].parameters?.map((p) => transformOperationParameter(p, context)) ?? [];
        ...
        for (const verb of supportedHttpMethods) {
            ...
            const splitOperations = splitOperationByContentType(operationId, decorators, dedupeParameters([...routeParameters, ...parameters]), operation.description, requestBodies, operationResponses, tags, fixmes, usedOperationIds);
...
function transformOperationParameter(parameter, context) {
    if ("$ref" in parameter) {
    ...
    const directives = "schema" in parameter && parameter.schema ? getDirectivesForSchema(parameter.schema) : [];
    return {
 ...
 */
function splitOperationByContentType(operationId, decorators, parameters, doc, requestBodies, responses, tags, fixmes, usedOperationIds) {
    ...
    const getContentTypeSuffix = (contentType) => {
