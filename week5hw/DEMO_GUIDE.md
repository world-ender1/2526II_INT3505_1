...
Mở trình duyệt Web (Chrome/Edge), phóng to màn hình, gõ đường dẫn:
👉 `http://127.0.0.1:5000/api/offset?page=50000`

> **Bạn nói gì lúc này:** *"Bây giờ em sẽ lật tới trang thứ 50.000 bằng phương pháp cũ (Method Offset). Hãy chú ý dòng chữ `execution_time_ms`. MongoDB mất tới hơn 600ms (nửa giây). Lý do là vì nó phải quét và đếm bỏ chạy rỗng từ điểm số 1 tới điểm 999.980."*

*Lưu ý thao tác: Sau đó bạn hãy bôi đen dòng chữ `_id` của thằng ở cuối cùng trong danh sách và copy lại.*

## Bước 4: Cú chốt (Phô diễn tốc độ khủng khiếp của Cursor)
Mở một tab trình duyệt khác, dán đường dẫn API Cursor trỏ đúng vào ID bạn vừa copy:
👉 `http://127.0.0.1:5000/api/cursor?cursor_id=<ID_VỪA_COPY>`

> **Bạn nói gì lúc này:** *"Bây giờ thay vì đếm chay, em sẽ dùng phương pháp Cursor. Em lấy toạ độ ID của thằng cuối cùng để làm cột mốc. CSDL giờ đây đi thẳng vào cây B-Tree Index và xuất kết quả ra ngay lặp tức. Xin thầy và lớp nhìn vào chỉ số `execution_time_ms`."*

*(Kết quả hiển thị trên màn hình trình duyệt lúc này sẽ chỉ là `1.2ms`, nhanh hơn gấp vài trăm lần).*
